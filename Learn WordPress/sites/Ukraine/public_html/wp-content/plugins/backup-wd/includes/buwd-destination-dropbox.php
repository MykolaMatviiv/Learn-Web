...
class Buwd_Destination_Dropbox
{
    ...
    public function get_options($job_id)
    {
        ...
        if ($dboxtoken && isset($_GET['auth_delete']) && (int)$_GET['auth_delete'] == 1) {
            try {
                if ($dboxtype == 'sandbox') {
                    $dropbox_app = new DropboxApp($this->sandbox_app_key, $this->sandbox_secret_key, $dboxtoken);
                } else {
        ...
        $options = array(
            ...
            'fields' => array(
                ...
                array(
                    ...
                    'value'      => '',
                    'hint'       => array(
                        'html' => '<button class="buwd-button ' . $disabled . '" onclick="get_auth_url(\'sandbox\'); return false;">Get Dropbox App auth code</button><p class="description">Press <b>Get Dropbox App auth code</b> to let the plugin create a dedicated folder titled BackupWD in Apps folder of your Dropbox account. The plugin will have permissions to read and write files into this folder only. You can also specify a subfolder as backup destination, using <b>Folder in Dropbox</b> setting.</p>' . $dbox_dropbox_sep_text
                    ),
                ...
                array(
                    ...
                    'value'      => '',
                    'hint'       => array(
                        'html' => '<button class="buwd-button ' . $disabled . '" onclick="get_auth_url(\'dropbox\'); return false;">Get Dropbox App auth code</button><p class="description">Press <b>Get Dropbox App auth code</b> to let the plugin create a dedicated folder titled BackupWD in Apps folder of your Dropbox account. The plugin will have permissions to read and write files into this folder only. You can also specify a subfolder as backup destination, using <b>Folder in Dropbox</b> setting.</p>'
                    ),
                    'visibility' => $auth_visibility
                ),
                array(
    ...
    {
        $this->sandbox_app_key = Buwd_Options::getSetting('drop_sandbox_key');
        $this->sandbox_secret_key = Buwd_Encrypt::decrypt(Buwd_Options::getSetting('drop_sandbox_secret'));
